...
- **Card issuance:** StraitsX MCP card gateway from the reference stack

Do not introduce a new dependency without saying why in one sentence.

## The reference stack

StraitsX published `straitsX-mcp-demo`, a reference repo containing an agent-side card gateway (a pure x402 client that signs EIP-3009 `transferWithAuthorization` for XSGD), a demo store, a conformance suite, and a shopping agent example. It talks to the real StraitsX sandbox.

Commands:
- `npm run setup` copies `.env`, generates the agent key, prints the address only
- `npm run stack` runs the demo store on :4030 and the gateway on :4010
- `npm run doctor` prints a health table for env, MCP, a 402 probe, RPC, and XSGD balance

**Reuse their gateway. Do not reimplement x402 or EIP-3009.**

## Network

| Setting | Value |
|---|---|
| Profile in use | FILL IN: sandbox-live (Fuji) or mainnet |
...
| My EVM address | FILL IN |

The track rules say C-Chain Mainnet. The reference repo defaults to Fuji. Confirm with organisers.
...
3. Wire to StraitsX MCP so an approved intent mints a real sandbox card
